...
pub struct FftResampler<F: Float> {
    fft_size_in: usize,
    fft_size_out: usize,
...
impl<F: Float + FftNum> FftResampler<F> {
    ...
    #[nonblocking]
    pub fn resample_unit(&mut self, input: &[F], output: &mut [F], overlap: &mut [F]) {
        ...
        for frame in 0..self.fft_size_in {
            // Copy input to input buffer
            self.input_buf[frame] = input[frame];
            // clear padding area
            self.input_buf[self.fft_size_in + frame] = F::zero();
        }
        ...
        for frame in 0..new_len {
            self.input_f[frame] = self.input_f[frame] * self.filter_f[frame];
        }

        // copy to modified spectrum
        self.output_f[..new_len].copy_from_slice(&self.input_f[..new_len]);
        self.output_f[new_len..].fill(Complex::zero());
        ...
        for frame in 0..self.fft_size_out {
            output[frame] = self.output_buf[frame] + overlap[frame];
        }
