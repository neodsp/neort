...
impl<S: Sample> ResamplerFixedIn<S> {
    ...
    pub fn new(
        ...
        let input_buffers: Vec<Vec<S>> =
            vec![vec![S::zero(); num_frames_in + fft_size_in]; num_channels as usize];
...
impl<S: Sample> Resampler<S> for ResamplerFixedIn<S> {
    #[rtsan::nonblocking]
    fn process(
        &mut self,
