...
pub struct RingbufferShared<S: Sample> {
    ringbuffers: Vec<SharedRb<Heap<S>>>,
}

impl<S: Sample> Default for RingbufferShared<S> {
    fn default() -> Self {
        Self {
            ringbuffers: Vec::new(),
        }
...
impl<S: Sample> Ringbuffer<S> for RingbufferShared<S> {
